...
sys.path.append("/reg/g/pcds/pyps/apps/hutch-python/xpp/dev/devpath/pcdsdevices/")
from lxe import*

def tt_rough_FB(ttamp_th = 0.04, ipm2_th = 2000, tt_window = 0.1,kp = 0.2,ki = 0.1,kd = 1):
        ...
        while(1):
            ...
            while(dlen < 121):
                ...
                if (ttamp > ttamp_th)and(ipm2val > ipm2_th)and(ttfwhm < 130)and(ttfwhm >  70)and(current_tt != tenshots_tt[-1,])and(txt.moving == False):# for filtering the last one is for when DAQ is stopping
...
def drift_log(idata):
    savefilename = "/cds/home/opr/xppopr/experiments/xpplu9818/drift_log_APR17.txt"
...
def tt_rough_FB(ttamp_th = 0.04, ipm2_th = 2000, tt_window = 0.1,kp = 1.2,ki = 0.0001,kd = 1):#ttamp:timetool signal amplitude threshold, imp2 I0 value threshold, tt_window signal width
    fbvalue = 0 # for drift record
    ave_tt = np.zeros([2,])
         while(1):
             ...
             pt = 0#time to get the good singal for PI"D"
             while(dlen < 121):
                 current_tt, ttamp, ipm2val, ttfwhm = get_ttall()
                 if(dlen%60 == 0):
                     print("tt_value",current_tt,"ttamp",ttamp,"ipm2",ipm2val)
                 if (ttamp > ttamp_th)and(ipm2val > ipm2_th)and(ttfwhm < 130)and(ttfwhm >  70)and(current_tt != tenshots_tt[-1,])and(txt.moving == False):# for filtering the last one is for when DAQ is stopping
                     tenshots_tt = np.insert(tenshots_tt,dlen,current_tt)
             ...
             ave_tt[0,] = np.mean(tenshots_tt)
             print("Moving average of timetool value:", ave_tt)
             fb_val = pid_control(kp,ki,kd,ave_tt,pt)#calculate the feedback value
             if(round(lxt(),13)==round(txt(),13) and (txt.moving == False)):#check not lxt or during motion of lxt_ttc
                 ave_tt_second=-((fb_val)*1e-12)
                 matlabPV_FB(ave_tt_second)
                 print("feedback %f ps"%fb_val)
...
def get_ttall():#get timetool related signal
    ...
    ipm2val = ttdata[3,]
    ttfwhm = ttdata[5,]
    return current_tt, ttamp, ipm2val, ttfwhm

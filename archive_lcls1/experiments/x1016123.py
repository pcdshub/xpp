...
class User():
    ...
    def nanoprobe_2D(self, xStart, xDelta,xpoint,ypoint,yDelta,datacollection):
        """ 
        simple rastering for running at 120Hz with shutter open/close before
        and after motion stop.
        ...
        #sweeptime = abs(xStart-xEnd)/7
        daq.connect()
        daq.begin_infinite(record = True)
        time.sleep(2)
        print('Reached horizontal start position')
        # looping through n round trips
        for i in range(round(ypoint*0.5)):
            
            for j in range(xpoint):
                sam_x.umvr(xDelta)
                time.sleep(datacollection)
            sam_z.umvr(yDelta)
            for ii in range(xpoint):
                sam_x.umvr((-1*xDelta))
                time.sleep(datacollection)
            sam_z.umvr(yDelta)

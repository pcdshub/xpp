...
sam_x = IMS(name='sam_x',prefix='XPP:USR:MMS:21')
sam_y = IMS(name='sam_y',prefix='XPP:USR:MMS:01')
...
class User():
    def __init__(self):
        ...
        # pulsed power trigger
        self.evr_pulse = Trigger('XPP:R32:EVR:32:TRIG5', name='evr_pulse')

        # store number of shots before and after
        self.nShotsPre = 1
        self.nShotsPost = 1
        self.nominal_zero = 894774.0

        return


    # ######### Sequencer and pulse picker ################################
    def prepare_sequence(self, nShotsPre=1, nShotsOn=1, nShotsPost=1):
        # update shots
        self.nShotsPre = nShotsPre
        self.nShotsPost = nShotsPost

        # hard code only one shot on
        #nShotsOn = 1
        ...
        preLine = [190, 0, 0, 0]  # marker for x-rays before pulsed power
        onLine = [97, 0, 0, 0]  # pulsed power shot
        postLine = [193, 0, 0, 0]  # marker for x-rays after pulsed power
    ...
    def set_delay(self,ns_delay):
        self.evr_pulse.ns_delay.set(self.nominal_zero-ns_delay)

    def takePulseRun(self,record=True):
        totalShots = self.nShotsPre + self.nShotsPost
        self.takeRun(totalShots,record=record)
    ...
    def dumbSnake(self, xStart, xEnd, yDelta, nRoundTrips):
        """ 

...
#            self.sam_x = IMS('XPP:USR:PRT:MMS:27', name='sam_x')
    ...
    def move_unfocused(self):
        att(1e-6)
        self.crl_x.umv_SD_clear()
        self.crl_y.umv_SD_clear()
        att(1)
        print('CRL OUT.')

    def move_focused(self):
        att(1e-6)
        self.crl_x.umv_SD_focus()
        self.crl_y.umv_SD_focus()
        att(0.1)
        print('CRL IN.')

    def move_zyla(self):
        att(1e-6)
        self.sam_x.umv_SD_test_zyla()
        att(0.1)
        print ('zyla in, att 0.1')

    def move_silica(self):
        att(1e-6)
        self.sam_x.umv_SD_test_silica()
        att(0.1)
        print ('silica in, att 0.1')


    def print_JJ(self):
        print ("JJ1 {:.2f} {:.2f}".format(self.jjhg.wm(),self.jjvg.wm()))
